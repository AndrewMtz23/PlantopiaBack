...
const ensureUserAddressColumns = async (db) => {
  ...
  const [rows] = await dbPromise.query(
    `SELECT COLUMN_NAME
     ...
     WHERE TABLE_SCHEMA = DATABASE()
       AND TABLE_NAME = 'tusuarios'
       AND COLUMN_NAME IN ('ciudad', 'estadoDireccion', 'codigoPostal', 'referenciasDomicilio', 'latitud', 'longitud')`
  );
  ...
  if (!existingColumns.has("latitud")) {
    await dbPromise.query(
      "ALTER TABLE tusuarios ADD COLUMN latitud DECIMAL(10,8) NULL AFTER referenciasDomicilio"
    );
    console.log("Columna tusuarios.latitud agregada correctamente.");
  }

  if (!existingColumns.has("longitud")) {
    await dbPromise.query(
      "ALTER TABLE tusuarios ADD COLUMN longitud DECIMAL(11,8) NULL AFTER latitud"
    );
    console.log("Columna tusuarios.longitud agregada correctamente.");
  }
};
